...
public sealed class MainForm : Form
{
    ...
    private void InitializeUi()
    {
        ...
        _lblAd = MakeStatusRow(statusGrid, 3, "중앙 광고");
    ...
    {
        _busy = busy;
        foreach (var b in new[] { _btnAuto, _btnBrowse, _btnTest, _btnRefresh, _btnConnect, _btnBlock, _btnRestore, _btnApplyAll, _btnDiag })
            b.Enabled = !busy;
        UseWaitCursor = busy;
    }
    ...
    private Task RefreshDevicesAsync() => RunBusyAsync(async () =>
    {
    ...
    {
        if (_adb is null) { ShowError("ADB 없음", "먼저 adb.exe를 찾거나 선택하십시오."); return; }
        var ep = _txtEndpoint.Text.Trim();
        if (string.IsNullOrWhiteSpace(ep))
        {
            ShowError("입력 필요", "연결할 주소:포트 를 입력하십시오. 예) 127.0.0.1:16384");
            return;
        }
        Log($"adb connect {ep}");
    ...
    private Task UpdateSelectedStatusAsync() => RunBusyAsync(async () =>
    {
    ...
    private Task ApplyBlockAsync() => RunBusyAsync(async () =>
    {
        ...
        Log(msg.Replace('\n', ' '));
        if (!ok) { ShowError("적용 실패", msg); await UpdateSelectedStatusAsync(); return; }
    ...
    private Task ApplyRestoreAsync() => RunBusyAsync(async () =>
    {
        ...
        Log(msg.Replace('\n', ' '));
        if (!ok) { ShowError("복원 실패", msg); await UpdateSelectedStatusAsync(); return; }
